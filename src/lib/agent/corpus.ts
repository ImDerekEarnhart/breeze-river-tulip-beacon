...
export const CORPUS: CorpusDoc[] = [
  ...
  },
  {
    ...
    tags: ["architecture", "retrieval", "tower", "routing"],
  ...
  },
  {
    ...
    tags: ["retrieval", "rag", "embeddings", "rerank", "tower"],
  ...
  },
  {
    ...
    tags: ["tower", "manifesto", "governance", "orbl", "superintelligence"],
  ...
  },
  {
    ...
    tags: ["fiber", "representation", "hole", "auditor", "factorization"],
  ...
  },
  {
    ...
    tags: ["flm", "representation", "refine", "quotient", "admission"],
  ...
  {
    id: "doc-handoff",
    title: "Guided executable systems handoff",
    tags: ["handoff", "flm", "orb1", "receipts", "quarantine"],
    text: `GUIDED_EXECUTABLE_SYSTEMS_HANDOFF is not a standalone app and does not contain Hodgeform Core. Restore Guided first. Diff overlay modules individually. Keep newer auth, PWA, routing, and scaffold. Hodgeform MCP/OAuth/protocol adapters are reference candidates only until inspected — never blind-overwrite. FLM candidate creation is non-mutating and requires separate exact-hash admission. ORB-1 quarantine is fail-closed outside Q(i). guided-receipt/1 is interchange, not Core approval. Optional ARC falsifier, HA-IR, and frozen spectral-fiber trees stay quarantined and unwired. They are not official ARC-AGI-3 or Opaque Fiber v1.0.1 scores.`,
  },
  {
    id: "doc-world",
    title: "The world persists when a conversation ends",
    tags: ["world", "persistence", "memory", "goals"],
    text: `Major ingredient: persistence. Chat and world stay together. There is no clear topic and no chat wipe. The saved world keeps position, body state, possessions, memory refs, relationships, unfinished goals, learned map, recent events, and conversation turns. Schema guided-world/1. Local to this preview. Not Hodgeform Core. Not a Tower VM. Student GPU and Core OAuth remain blocked goals until supplied. Origin is still not evidence: a run event may cite a receipt hash, but that is not Core approval.`,
  },
  {
    id: "doc-orbita",
  ...
  },
  {
    ...
    tags: ["receipts", "evidence", "loop", "hash"],
    text: `Guided receipts are an interchange, not a second scientific engine. Schema guided-receipt/1. Kinds: context, proposal, execution, verification, admission, control. A SHA-256 hash binds the canonical body excluding the hash field. parentReceiptIds form an evidence chain. Retrieve emits a context receipt over BM25. A student or teacher structured action emits a proposal receipt with coreApproved=false — it does not freeze, admit, or execute. Tools emit execution receipts without changing tool semantics. Verify emits a verification receipt parented to the most recent prior receipt. Hodgeform Core remains the Control plane. These hashes do not replace Core event or artifact hashes, do not freeze plans, and do not approve anything.`,
  },
  {
    id: "doc-orb1",
    title: "ORB-1: Q(i) coefficient ring, not a language theorem",
    tags: ["orb1", "gaussian", "admission", "quarantine"],
  ...
  {
    id: "doc-education-verifier",
    title: "Local education verifier, not Core LANGUAGE_LIMIT",
    tags: ["verifier", "education", "retry"],
    text: `The architecture loop diagnosed VERIFIER_LIMIT: the fast-path check was presence, length, confidence, and a permissive numeric pass. Guided adds a local education scorer over a frozen English smoke suite. Missing answers fail. High-confidence wrong labels fail. Teacher corrections stay quarantined until that scorer independently passes the teacher answer. A student-only exam with no student fail-closes and does not call the teacher. An NLP miss cannot issue LANGUAGE_LIMIT. This suite is operator-visible smoke, not a sealed held-out benchmark, not Core admission, and not activation of candidate 53f3b10f.`,
  },
];
